...
WelcomeChat::WelcomeChat(wxWindow *parent, MainFrame *mainFrame)
    : wxPanel(parent, wxID_ANY), m_mainFrame(mainFrame),
      m_telegramClient(nullptr), m_chatArea(nullptr),
      m_state(LoginState::NotStarted), m_codeRetries(0) {
  CreateUI();
  // Note: AppendWelcome() is NOT called here - it will be called by
  // MainFrame::ApplySavedFonts() after the correct font is set
}
...
void WelcomeChat::RefreshDisplay() {
  if (!m_chatArea)
    return;

  // Only refresh if we're in the initial state (showing welcome text)
  // Don't clear if user is in the middle of login flow
  if (m_state == LoginState::NotStarted) {
    m_chatArea->Clear();
    AppendWelcome();
  }
  // For other states, the font change will apply to new text only
}

void WelcomeChat::InitialDisplay() {
  // Called by MainFrame after fonts are properly configured
  if (m_chatArea && m_state == LoginState::NotStarted) {
    AppendWelcome();
  }
}
...
void WelcomeChat::AppendWelcome() {
  ...
  m_chatArea->BeginTextColour(m_chatArea->GetInfoColor());
  m_chatArea->WriteText(
      "* Welcome to Teleliter - Telegram client with HexChat interface\n");
  m_chatArea->EndTextColour();
  ...
  m_chatArea->GetDisplay()->Refresh();
  m_chatArea->GetDisplay()->Update();
...
void WelcomeChat::StartLogin() {
  ...
  // Start TelegramClient if not running (shouldn't happen as we start on app
  // launch)
...
bool WelcomeChat::IsWelcomeChatCommand(const wxString &command) const {
  ...
  return cmd == "/login" || cmd == "/cancel" || cmd == "/quit" ||
         cmd == "/exit";
}
...
bool WelcomeChat::ProcessInput(const wxString &input) {
  ...
  if (trimmed.StartsWith("/")) {
    ...
    if (command == "/login") {
      ...
      // In NotStarted state, show WelcomeChat help but also let regular help
      // through
      m_chatArea->AppendUserInput(trimmed);
      ...
      m_chatArea->AppendInfo("Other commands like /me, /clear, /whois work "
      ...
      return true;
    } else if (m_state == LoginState::NotStarted ||
               m_state == LoginState::LoggedIn) {
...
void WelcomeChat::HandlePhoneInput(const wxString &input) {
  if (!ValidatePhoneNumber(input)) {
    m_chatArea->AppendError("Invalid phone number format. Please include "
...
void WelcomeChat::HandleCodeInput(const wxString &input) {
  if (!ValidateCode(input)) {
    ...
    m_chatArea->AppendError(
        "Invalid code format. Please enter the numeric code.");
...
void WelcomeChat::OnAuthStateChanged(int state) {
  ...
  case AuthState::WaitPhoneNumber:
    ...
    if (m_state == LoginState::NotStarted) {
      ...
      m_chatArea->AppendSuccess(
          "Connected to Telegram. Type /login to sign in.");
...
void WelcomeChat::OnLoginSuccess(const wxString &userName,
                                 const wxString &phoneNumber) {
