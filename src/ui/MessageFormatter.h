...
public:
  ...
  void AppendEditedMessage(const wxString &timestamp, const wxString &sender,
                           const wxString &message,
                           long *editSpanStart = nullptr,
                           long *editSpanEnd = nullptr,
                           MessageStatus status = MessageStatus::None,
                           bool statusHighlight = false);
  ...
  // Remove the unread marker if present (call when new messages arrive while
  // viewing)
  void RemoveUnreadMarker();

  // Reset marker tracking without trying to remove text (call when chat is
  // cleared)
  void ResetUnreadMarker() {
  ...
  void AppendContinuationMessage(const wxString &timestamp,
                                 const wxString &message,
                                 MessageStatus status = MessageStatus::None,
                                 bool statusHighlight = false);

  // Continuation media message
  void AppendContinuationMediaMessage(
      const wxString &timestamp, const MediaInfo &media,
      const wxString &caption = "", MessageStatus status = MessageStatus::None,
      bool statusHighlight = false);
  ...
  void
  AppendReactions(const std::map<wxString, std::vector<wxString>> &reactions);

  // Check if we should group with previous message (same sender within time
  // window)
...
private:
  ...
  static const int GROUP_TIME_WINDOW_SECONDS =
      120; // 2 minutes for tighter grouping
  ...
  void WriteTextWithLineHandling(const wxString &text,
                                 MessageStatus status = MessageStatus::None,
                                 bool statusHighlight = false);
  ...
  static wxString
  GenerateAsciiWaveform(const std::vector<uint8_t> &waveformData,
                        int targetLength);
};
