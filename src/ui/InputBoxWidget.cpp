...
InputBoxWidget::InputBoxWidget(wxWindow *parent, MainFrame *mainFrame)
    : wxPanel(parent, wxID_ANY), m_mainFrame(mainFrame), m_chatView(nullptr),
      m_memberList(nullptr), m_messageFormatter(nullptr),
      m_welcomeChat(nullptr), m_inputBox(nullptr), m_uploadBtn(nullptr),
...
void InputBoxWidget::CreateLayout() {
  ...
  m_inputBox = new wxStyledTextCtrl(this, wxID_ANY, wxDefaultPosition,
  ...
  m_inputBox->StyleSetForeground(
      wxSTC_STYLE_DEFAULT, wxSystemSettings::GetColour(wxSYS_COLOUR_GRAYTEXT));
  m_inputBox->StyleClearAll();
...
void InputBoxWidget::CreateButtons() {
  ...
  m_uploadBtn = new wxButton(this, ID_UPLOAD_BTN, "Upload", wxDefaultPosition,
                             wxDefaultSize);
...
void InputBoxWidget::OnUploadPhoto(wxCommandEvent &event) {
  ...
  wxFileDialog dlg(this, "Select photo to upload", "", "",
                   "Images "
                   "(*.jpg;*.jpeg;*.png;*.gif;*.webp;*.bmp)|*.jpg;*.jpeg;*.png;"
                   "*.gif;*.webp;*.bmp|"
                   "All Files (*.*)|*.*",
...
void InputBoxWidget::OnUploadVideo(wxCommandEvent &event) {
  ...
  wxFileDialog dlg(this, "Select video to upload", "", "",
                   "Videos "
                   "(*.mp4;*.mkv;*.avi;*.mov;*.webm;*.wmv)|*.mp4;*.mkv;*.avi;*."
                   "mov;*.webm;*.wmv|"
                   "All Files (*.*)|*.*",
                   wxFD_OPEN | wxFD_FILE_MUST_EXIST | wxFD_MULTIPLE);
...
void InputBoxWidget::OnUploadFile(wxCommandEvent &event) {
  ...
  if (chatId == 0) {
    if (m_messageFormatter) {
      m_messageFormatter->AppendServiceMessage(
          GetCurrentTimestamp(), "Select a chat first to upload files");
    }
    return;
  }
  ...
  wxFileDialog dlg(
      ...
      "All Files (*.*)|*.*|"
      "Documents "
      "(*.pdf;*.doc;*.docx;*.xls;*.xlsx;*.txt)|*.pdf;*.doc;*.docx;*.xls;*.xlsx;"
      "*.txt|"
...
void InputBoxWidget::OnTextChanged(wxStyledTextEvent &event) { event.Skip(); }
...
void InputBoxWidget::ProcessMeCommand(const wxString &args) {
  ...
  if (m_messageFormatter) {
    m_messageFormatter->AppendActionMessage(GetCurrentTimestamp(), sender,
                                            args);
  }
...
void InputBoxWidget::ProcessQueryCommand(const wxString &args) {
  if (!args.IsEmpty()) {
    ...
    if (m_messageFormatter) {
      m_messageFormatter->AppendServiceMessage(
          GetCurrentTimestamp(), "Usage: /query <username> [message]");
    }
  }
}
...
void InputBoxWidget::ProcessTopicCommand(const wxString &args) {
  if (!args.IsEmpty()) {
    ...
    if (m_messageFormatter) {
      m_messageFormatter->AppendServiceMessage(
          GetCurrentTimestamp(), "Topic: (use /topic <text> to set)");
    }
  }
}
...
void InputBoxWidget::ProcessWhoisCommand(const wxString &args) {
  if (!args.IsEmpty()) {
    wxString target = args.BeforeFirst(' ');
    if (m_messageFormatter) {
      m_messageFormatter->AppendNoticeMessage(
          GetCurrentTimestamp(), "Teleliter", "Looking up " + target + "...");
      // TODO: Get user info via TDLib
      m_messageFormatter->AppendNoticeMessage(
          GetCurrentTimestamp(), "Teleliter", target + " is a Telegram user");
    }
  } else {
    if (m_messageFormatter) {
      m_messageFormatter->AppendServiceMessage(GetCurrentTimestamp(),
...
void InputBoxWidget::DoTabCompletion() {
  ...
  wxString newText =
      text.Left(wordStart) + completion + text.Mid(insertionPoint);
  ...
  if (wxTheClipboard->IsSupported(wxDF_BITMAP)) {
    ...
    if (wxTheClipboard->GetData(bitmapData)) {
      ...
      if (bitmap.IsOk() && m_mainFrame) {
        // Use unique filename with timestamp and counter to prevent
        // race conditions when pasting multiple images rapidly.
        // Each paste gets a distinct file path so uploads don't collide.
        static int s_pasteCounter = 0;
        wxString tempPath =
            wxFileName::GetTempDir() +
            wxString::Format("/teleliter_paste_%lld_%d.png",
