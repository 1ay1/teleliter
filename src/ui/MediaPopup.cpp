...
wxBEGIN_EVENT_TABLE(MediaPopup, wxPopupWindow) EVT_PAINT(MediaPopup::OnPaint)
    ...
    : wxPopupWindow(parent, wxBORDER_NONE), m_hasImage(false),
      ...
      m_mediaCtrl(nullptr), m_isPlayingVideo(false), m_loopVideo(false),
      m_videoMuted(true), m_isPlayingSticker(false),
      m_lottieAnimTimer(this, LOTTIE_ANIM_TIMER_ID), m_isPlayingWebm(false),
      m_webmAnimTimer(this, WEBM_ANIM_TIMER_ID), m_isPlayingFFmpeg(false),
      m_ffmpegAnimTimer(this, FFMPEG_ANIM_TIMER_ID),
      m_asyncLoadTimer(this, ASYNC_LOAD_TIMER_ID), m_asyncLoadPending(false),
      m_parentBottom(-1), m_videoLoadStartTime(0) {
...
void MediaPopup::PlayAnimatedSticker(const wxString &path) {
  ...
  if (HasFailedRecently(path)) {
    MPLOG("PlayAnimatedSticker: skipping recently failed sticker: "
          << path.ToStdString());
    FallbackToThumbnail();
    return;
  }
  ...
  m_lottiePlayer->SetRenderSize(STICKER_MAX_WIDTH - PADDING * 2,
                                STICKER_MAX_HEIGHT - PADDING * 2 - 20);
...
void MediaPopup::PlayWebmSticker(const wxString &path) {
  ...
  MPLOG("PlayWebmSticker: using FFmpegPlayer on Linux for "
        << path.ToStdString());
  ...
  m_webmPlayer->SetRenderSize(STICKER_MAX_WIDTH - PADDING * 2,
                              STICKER_MAX_HEIGHT - PADDING * 2 - 20);
  m_webmPlayer->SetLoop(true);

  // Set callback to receive frames
  m_webmPlayer->SetFrameCallback(
      [this](const wxBitmap &frame) { OnWebmFrame(frame); });
...
void MediaPopup::CreateMediaCtrl() {
  ...
  if (!m_mediaCtrl->Create(
          this, wxID_ANY, wxEmptyString,
          wxPoint(PADDING + BORDER_WIDTH, PADDING + BORDER_WIDTH),
          wxSize(PHOTO_MAX_WIDTH - PADDING * 2,
                 PHOTO_MAX_HEIGHT - PADDING * 2 - 24),
          wxMC_NO_AUTORESIZE)) {
    delete m_mediaCtrl;
    m_mediaCtrl = nullptr;
    return;
  ...
  m_mediaCtrl->Bind(wxEVT_MEDIA_STATECHANGED, &MediaPopup::OnMediaStateChanged,
                    this);
}
...
void MediaPopup::DestroyMediaCtrl() {
  if (m_mediaCtrl) {
    ...
    m_mediaCtrl->Unbind(wxEVT_MEDIA_FINISHED, &MediaPopup::OnMediaFinished,
                        this);
    m_mediaCtrl->Unbind(wxEVT_MEDIA_STOP, &MediaPopup::OnMediaStop, this);
    m_mediaCtrl->Unbind(wxEVT_MEDIA_STATECHANGED,
                        &MediaPopup::OnMediaStateChanged, this);
    m_mediaCtrl->Destroy();
    m_mediaCtrl = nullptr;
  }
  m_isPlayingVideo = false;
...
void MediaPopup::ShowMedia(const MediaInfo &info, const wxPoint &pos) {
  ...
  if (IsShown() && IsSameMedia(m_mediaInfo, info)) {
    ...
    MPLOG("ShowMedia: same media check - localPathChanged="
          << localPathChanged
  ...
  // If upgrading the same file (e.g. thumbnail -> full), keep the old image
  // until new one loads
  ...
  if (info.type == MediaType::Sticker) {
    ...
    StickerFormat format = hasLocalFile ? DetectStickerFormat(info.localPath)
    ...
    // Fall back to thumbnail for preview (thumbnails are usually static
    // WebP/JPEG)
    if (!info.thumbnailPath.IsEmpty() && wxFileExists(info.thumbnailPath)) {
      MPLOG("ShowMedia: sticker - falling back to thumbnail: "
            << info.thumbnailPath.ToStdString());
      // Show loading placeholder immediately, then load image async
      m_isLoading = true;
      m_loadingFrame = 0;
      m_loadingTimer.Start(150);
    ...
    if (info.fileId != 0 &&
        (info.localPath.IsEmpty() || !wxFileExists(info.localPath))) {
      MPLOG("ShowMedia: sticker file not ready, showing loading for fileId="
            << info.fileId);
      m_isLoading = true;
      m_loadingFrame = 0;
      m_loadingTimer.Start(150);
  ...
  // If not marked as downloading in info, double check with TelegramClient if
  // we have a frame
  if (!isTrulyDownloading && info.fileId != 0) {
    // Can't easily check client state from here without access to main frame or
    // client But we can trust info.isDownloading if ChatViewWidget kept it up
    // to date However, if localPath is empty but isDownloading is false, it
    // might mean:
    // 1. Not started yet
    ...
    // But to avoid the "starts out showing downloading" glitch for already
    // downloaded files, we should start with a PLACEHOLDER unless we are SURE
    // it is downloading.

    // BETTER APPROACH: Use the thumbnail size for the placeholder to avoid
    // jumping even if we show a spinner.
  }

  if (info.isDownloading || info.fileId != 0) {
    MPLOG("ShowMedia: no local file, showing downloading state for fileId="
          << info.fileId);
    m_isDownloadingMedia = true;
    m_isLoading = true;
    m_loadingFrame = 0;
    ...
    if (info.width > 0 && info.height > 0) {
      ...
      if (info.type != MediaType::Photo && info.type != MediaType::Video &&
...
void MediaPopup::PlayVideo(const wxString &path, bool loop, bool muted) {
  MPLOG("PlayVideo called: path=" << path.ToStdString() << " loop=" << loop
                                  << " muted=" << muted);
  ...
  m_isPlayingVideo = false;
  m_hasImage = false;
  m_videoLoadStartTime =
      wxGetUTCTimeMillis().GetValue(); // Track when we started loading
  ...
  // Use single source of truth for size and position - this also shows the
  // window
  ...
  if (!m_mediaCtrl) {
    ...
    MPLOG(
        "PlayVideo: failed to create media control, falling back to thumbnail");
    FallbackToThumbnail();
    return;
  }
...
void MediaPopup::PlayVideoWithFFmpeg(const wxString &path, bool loop,
                                     bool muted) {
#ifdef HAVE_FFMPEG
...
void MediaPopup::FallbackToThumbnail() {
  ...
  if (!m_mediaInfo.thumbnailPath.IsEmpty() &&
    ...
    // LoadImageAsync -> SetImage -> UpdateSize -> ApplySizeAndPosition handles
    // positioning
    Refresh();
...
void MediaPopup::OnMediaLoaded(wxMediaEvent &event) {
  ...
  if (m_mediaCtrl) {
    ...
    if (videoSize.GetWidth() > 0 && videoSize.GetHeight() > 0) {
      ...
      if (vidWidth > PHOTO_MAX_WIDTH - PADDING * 2 ||
          vidHeight > PHOTO_MAX_HEIGHT - PADDING * 2 - 24) {
        double scaleX = (double)(PHOTO_MAX_WIDTH - PADDING * 2) / vidWidth;
        double scaleY =
            (double)(PHOTO_MAX_HEIGHT - PADDING * 2 - 24) / vidHeight;
        double scale = std::min(scaleX, scaleY);
...
void MediaPopup::OnMediaStateChanged(wxMediaEvent &event) {
  ...
  if (m_isLoading && state == wxMEDIASTATE_STOPPED) {
    ...
    if (elapsed >
        2000) { // If stopped after 2 seconds of loading, probably failed
      MPLOG("OnMediaStateChanged: video loading seems to have failed, falling "
            "back");
      m_loadingTimer.Stop();
      m_isLoading = false;
...
void MediaPopup::SetImage(const wxImage &image) {
  ...
  Refresh();
  MPLOG("SetImage: after ApplySizeAndPosition - pos="
...
void MediaPopup::OnLoadingTimer(wxTimerEvent &event) {
  ...
  if (m_isLoading && m_mediaCtrl && !m_isPlayingVideo) {
    ...
    if (elapsed > VIDEO_LOAD_TIMEOUT_MS) {
      MPLOG("OnLoadingTimer: video load timed out after "
            << elapsed << "ms, falling back to thumbnail");
      m_loadingTimer.Stop();
    ...
    // Continue running timer even if IsShown() is false (video loads in
    // background/separate window)
  } else if (!IsShown() && !m_isDownloadingMedia) {
    // Stop timer if popup is hidden and we're not waiting for video load or
    // download
    m_loadingTimer.Stop();
    return;
  }
...
void MediaPopup::AdjustPositionToScreen(const wxPoint &pos) {
  ...
  MPLOG("AdjustPositionToScreen: new originalPosition=(" << pos.x << ","
                                                         << pos.y << ")");

  // Force update of position with current size if already shown
  if (IsShown()) {
    wxSize size = GetSize();
    ApplySizeAndPosition(size.GetWidth(), size.GetHeight());
  }
...
// Single source of truth for setting size and position with smart screen-aware
// positioning
void MediaPopup::ApplySizeAndPosition(int width, int height) {
  MPLOG("ApplySizeAndPosition: requested size=("
        << width << "," << height << ")"
        << " m_originalPosition=(" << m_originalPosition.x << ","
  ...
  const int EDGE_MARGIN = 10; // Minimum margin from screen edges

  wxPoint targetPos = m_originalPosition;
  bool isShowingBelow = true;

  // Skip complex positioning if we don't have a valid anchor point
  if (m_originalPosition.x == 0 && m_originalPosition.y == 0) {
    // Fallback: just center on primary display
    ...
    // ===== STEP 1: Determine the display and screen bounds =====
    int displayIndex = wxDisplay::GetFromPoint(m_originalPosition);
    if (displayIndex == wxNOT_FOUND) {
      // Point is outside all monitors, find the closest one
      displayIndex = 0;
      int minDist = INT_MAX;
      for (unsigned int i = 0; i < wxDisplay::GetCount(); i++) {
        wxDisplay disp(i);
        wxRect rect = disp.GetClientArea();
        // Calculate distance to center of this display
    ...
    MPLOG("ApplySizeAndPosition: display="
          << displayIndex << " screenRect=(" << screenRect.GetLeft() << ","
          << screenRect.GetTop() << "," << screenRect.GetWidth() << ","
          << screenRect.GetHeight() << ")");

    // ===== STEP 2: Determine effective bounds =====
    // Use the full screen client area (accounts for taskbar/dock)
    ...
    MPLOG("ApplySizeAndPosition: effective bounds=("
          << effectiveLeft << "," << effectiveTop << ") to (" << effectiveRight
          << "," << effectiveBottom << ")");

    // ===== STEP 3: Calculate horizontal position =====
    // Align left edge with cursor (more natural for reading flow)
    ...
    // Shift right if it goes off the left edge (rare, but possible)
    if (targetPos.x < effectiveLeft) {
      targetPos.x = effectiveLeft;
    }
    // ===== STEP 4: Calculate vertical position =====
    // GOAL: Keep popup visually adjacent to cursor at all times
    // When showing below: popup top is just below cursor
    // When showing above: popup bottom is just above cursor

    const int SMALL_GAP = 5; // Minimal gap to feel "attached" to click point

    int spaceBelow = effectiveBottom - m_originalPosition.y - SMALL_GAP;
    int spaceAbove = m_originalPosition.y - effectiveTop - SMALL_GAP;

    MPLOG("ApplySizeAndPosition: spaceBelow=" << spaceBelow
                                              << " spaceAbove=" << spaceAbove
                                              << " height=" << height);

    if (height <= spaceBelow) {
      // Fits below cursor - preferred position
      targetPos.y = m_originalPosition.y + SMALL_GAP;
      isShowingBelow = true;
    } else if (height <= spaceAbove) {
      // Fits above cursor - position so bottom edge is just above click
      targetPos.y = m_originalPosition.y - height - SMALL_GAP;
      isShowingBelow = false;
    } else {
      // Doesn't fit well in either direction
      // PRIORITIZE keeping popup adjacent to cursor over full visibility
      if (spaceBelow >= spaceAbove) {
        // More space below - show below, letting bottom get clipped if needed
        targetPos.y = m_originalPosition.y + SMALL_GAP;
        isShowingBelow = true;
      } else {
        // More space above - position bottom edge just above cursor
        // This keeps the popup visually "attached" to where user clicked
        targetPos.y = m_originalPosition.y - height - SMALL_GAP;
        isShowingBelow = false;
      }
    }

    // ===== STEP 5: Gentle bounds clamping (prioritize cursor proximity) =====
    // Only clamp if we would go significantly off-screen
    // Allow partial overflow to keep popup near cursor

    // Clamp top - but allow up to 50px overflow to stay near cursor
    int minAllowedTop = effectiveTop - 50;
    if (targetPos.y < minAllowedTop) {
      targetPos.y = minAllowedTop;
    }
    // Hard clamp to actual screen edge
    if (targetPos.y < screenRect.GetTop()) {
      targetPos.y = screenRect.GetTop();
    }

    // Clamp bottom - shift up only if bottom would go way off screen
    if (targetPos.y + height > effectiveBottom + 30) {
      // Shift up, but try to keep the top near cursor when showing above
      int overshoot = (targetPos.y + height) - effectiveBottom;
      if (!isShowingBelow) {
        // When showing above, we want bottom near cursor, so limit how much we
        // shift
        overshoot =
            std::min(overshoot, height / 3); // Shift at most 1/3 of height
      }
    ...
    // Horizontal clamping (keep fully on screen horizontally)
    if (targetPos.x < effectiveLeft) {
      targetPos.x = effectiveLeft;
    }
    if (targetPos.x + width > effectiveRight) {
  ...
  MPLOG("ApplySizeAndPosition: FINAL target=("
        << targetPos.x << "," << targetPos.y << "," << width << "," << height
        << ") isShowingBelow=" << isShowingBelow);

  // ===== STEP 6: Apply the geometry (with platform-specific workarounds) =====
#ifdef __WXGTK__
  // On GTK, popup windows are notoriously difficult to reposition and resize.
  // The most reliable sequence is: hide, move to offscreen, set new geometry,
  // show.
  Hide();

  // Move to offscreen position first to force window manager to re-evaluate
  Move(-5000, -5000);
  wxYield();

  // Now set the actual size and position
  SetSize(targetPos.x, targetPos.y, width, height);

  // Show at new location
  Show();
#else
  // On macOS/Windows, simpler approach works
  if (IsShown()) {
    // If already visible, just resize and move
    SetSize(targetPos.x, targetPos.y, width, height);
  } else {
    SetSize(targetPos.x, targetPos.y, width, height);
    Show();
  ...
  // Force a layout and full refresh to ensure content is drawn correctly
  Layout();
  Refresh(true);
  Update();
  ...
  MPLOG("ApplySizeAndPosition: RESULT pos=("
...
void MediaPopup::UpdateSize() {
  ...
  if (m_hasImage && m_bitmap.IsOk()) {
    ...
    height = m_bitmap.GetHeight() + (PADDING * 2) + (BORDER_WIDTH * 2) +
             24; // +24 for label
...
void MediaPopup::OnPaint(wxPaintEvent &event) {
  ...
  if (m_hasImage && m_bitmap.IsOk()) {
    ...
    // If this is a thumbnail and the main file is still downloading, show
    // download indicator
    ...
    // Error state
    dc.SetTextForeground(
        wxColour(0xCC, 0x00, 0x00)); // Red for errors (semantic color)
