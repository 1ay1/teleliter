...
public:
  ...
  // Color control
  void BeginTextColour(const wxColour &color) {
    m_chatDisplay->BeginTextColour(color);
  }
  ...
  void WriteTimestamp(const wxString &timestamp, MessageStatus status,
  ...
  void AppendAction(const wxString &timestamp, const wxString &sender,
...
protected:
  ...
  bool m_wasAtBottom;
  int m_batchDepth;
  bool m_refreshPending; // True if a refresh is scheduled
